...
app.post('/jude branch', (req,res) => {
  if (!req.body || !req.body.name) {
    res.status(400).send("Missing a  in the request body!");
    return;
  }

  users.filter(name => name != req.body.name);
  res.send('User with name ${req.body.name} has been removed');
});